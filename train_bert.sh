...
export pretrain_dir=chinese-roberta-wwm-ext                                  # 预训练模型目录
#export lm=chinese_roberta_wwm_ext                                               # 原始预训练模型名称
...
python main_bert.py \
  ...
  --config_name $pretrain_dir/config.json \
  --tokenizer_name $pretrain_dir/vocab.txt \
  --threads 12 \
  --warmup_ratio 0.1 \
  --logging_ratio 0.1 \
  --save_ratio 0.1 \
